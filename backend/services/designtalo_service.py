...
from SQL_kyselyt import (
    ...
    tallenna_ai_hakutulokset_kantaan
)
...
class DesigntaloService:
    ...
    def process_document(self, pdf_path: str, toimitussisalto_id: int) -> dict:
        """
        Käsittele Designtalo-dokumentti
        ...
        """
        try:
            self.logger.info(f"Käsitellään Designtalo-dokumentti: {pdf_path}")
            
            # Käsittele PDF Designtalo-kohtaisesti
            doc_result = self.document_processor.process_pdf(pdf_path, "Designtalo")
            
            # Analysoi AI:lla (käytetään Sievitalo-analyysiä kunnes Designtalo-kohtainen on valmis)
            ...
            if analysis_result.get('tuotteet'):
                tallenna_ai_hakutulokset_kantaan(analysis_result['tuotteet'])
            ...
            self.logger.error(f"Virhe Designtalo-dokumentin käsittelyssä: {e}")
            raise
