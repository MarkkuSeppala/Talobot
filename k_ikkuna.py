    ...
    for item in json_data:
        ...
        for _ in range(item["kpl"]):
            output_json.append({
                ...
                "pyoristetty_leveys": pyoristetty_leveys,  # **Tämä lisättävä, jotta lajittelu toimii**  
                "turvalasi": item["turvalasi"],
                "välikarmi": item["välikarmi"],
                "sälekaihtimet": item["sälekaihtimet"]
            })

    # **Lajitellaan lista pyoristetty_leveys-kentän mukaan**
    output_json = sorted(output_json, key=lambda x: x["pyoristetty_leveys"])
    
    # Poistetaan väliaikainen "pyoristetty_leveys"-kenttä, koska sitä ei tarvita JSON-tiedostossa
    for item in output_json:
        del item["pyoristetty_leveys"]
