...
from api_kyselyt import api_kysely, api_kysely_kirjoitus_json, api_kysely_ulko_ovet, api_kysely_nelja_parametria, openai_api_kysely, openai_api_kysely_nelja_parametria, openai_api_kysely_ulko_ovet
...
def run_sievitalo(toimitussisalto_pdf, toimitussisalto_id: int):
    print(f"🚀 ALOITETAAN SIEVITALO-ANALYYSI (ID: {toimitussisalto_id})")
    logging.info(f"🚀 ALOITETAAN SIEVITALO-ANALYYSI (ID: {toimitussisalto_id})")
    ...
    print(f"📄 Syöte-pituus: {len(puhdistettu_toimitussisalto)} merkkiä")
    # Palauttaa tekstimuodossa listan toimitussisällön ikkunoista
    ikkunatiedot_kokonaisuudessa = openai_api_kysely(PROMPT_SIEVITALO_POIMI_IKKUNATIEDOT_TXT, puhdistettu_toimitussisalto)
    
    if ikkunatiedot_kokonaisuudessa and ikkunatiedot_kokonaisuudessa.strip():
        ...
        print(f"📄 Tulos-pituus: {len(ikkunatiedot_kokonaisuudessa)} merkkiä")
    else:
        print("❌ VIRHE: Sievitalo ikkuna-analyysi palautti tyhjän tuloksen!")
        kirjoita_txt_tiedosto("VIRHE: Tyhjä tulos Sievitalo ikkuna-analyysissä", f"terminaalitulosteet/sievitalo_ikkunatiedot_raaka_{toimitussisalto_id}.txt")

    print(f"\n🔍 SIEVITALO API-KYSELY: Ikkunat JSON-muotoon")
    # Palauttaa ikkunat JSON-muodossa
    ikkunat_json = openai_api_kysely(PROMPT_SIEVITALO_RYHMITELLE_VALITUT_IKKUNATIEDOT_JSON_MUOTOON, ikkunatiedot_kokonaisuudessa)
    ...
    ikkunat_json_puhdistettu = ikkunat_json.replace("```json", "").replace("```", "").strip()
        
    # Debug: Tarkista JSON-muoto
    print(f"🔍 Sievitalo ikkunat JSON (ensimmäiset 200 merkkiä): {ikkunat_json_puhdistettu[:200]}...")
    
    # Tarkista että JSON ei ole tyhjä
        if not ikkunat_json_puhdistettu or ikkunat_json_puhdistettu.strip() == "":
            print("❌ VIRHE: Sievitalo ikkunat JSON on tyhjä!")
            return
        
        # Lisätään ikkunat tietokantaan
        lisaa_ikkunat_kantaan_ja_koko_x_100(ikkunat_json_puhdistettu, toimitussisalto_id)
        ...
        ulko_ovet_teksti = openai_api_kysely(PROMPT_SIEVITALO_POIMI_ULKO_OVI_TIEDOT_TXT, puhdistettu_toimitussisalto)
        ...
        if not ulko_ovet_teksti or ulko_ovet_teksti.strip() == "":
            ...
            ulko_ovet = openai_api_kysely_ulko_ovet(PROMPT_SIEVITALO_ULKO_OVI_TIEDOT_LUOKKAMUOTOON, ulko_ovet_teksti)
       ...
       #---------------------------------------     Sievitalo vali-ovet kantaan      ----------------------------------------
        ...
        valio_ovet = openai_api_kysely(PROMPT_SIEVITALO_POIMI_VALIOVITIEDOT_TXT, puhdistettu_toimitussisalto)
        ...
        if not valio_ovet or valio_ovet.strip() == "":
            ...
            valio_ovet = openai_api_kysely(PROMPT_SIEVITALO_ANNA_VALIOVIMALLIT_TXT, valio_ovet)
        ...
        toimitussisalto_tuotteet = openai_api_kysely_nelja_parametria(PROMPT_POIMI_TUOTTEET_1_TXT, puhdistettu_toimitussisalto, tuotteet)
...
def run_kastelli(toimitussisalto_pdf, toimitussisalto_id: int):
    print(f"🚀 ALOITETAAN KASTELLI-ANALYYSI (ID: {toimitussisalto_id})")
    logging.info(f"🚀 ALOITETAAN KASTELLI-ANALYYSI (ID: {toimitussisalto_id})")
       ...
       #---------------------------------------     Kastelli ikkunat kantaan      ----------------------------------------
        ...
        # Debug: Tarkista että toimitussisältö on oikea
        print(f"📄 Toimitussisältö pituus: {len(puhdistettu_toimitussisalto)} merkkiä")
        print(f"📄 Toimitussisältö alku: {puhdistettu_toimitussisalto[:200]}...")
        print(f"📄 Toimitussisältö loppu: ...{puhdistettu_toimitussisalto[-200:]}")
        
        # Varmista että toimitussisältö sisältää oikeat tunnisteet
        if "**TOIMITUSSISÄLTÖ START**" not in puhdistettu_toimitussisalto:
            print("⚠️ VIRHE: Toimitussisältö ei sisällä **TOIMITUSSISÄLTÖ START** tunnistetta!")
        if "**TOIMITUSSISÄLTÖ END**" not in puhdistettu_toimitussisalto:
            print("⚠️ VIRHE: Toimitussisältö ei sisällä **TOIMITUSSISÄLTÖ END** tunnistetta!")
        
        print(f"🔍 Aloitetaan Kastelli ikkuna-analyysi...")
        print(f"📄 Syöte-pituus: {len(puhdistettu_toimitussisalto)} merkkiä")
        print(f"📄 Syöte alku: ...{puhdistettu_toimitussisalto[:100]}")
        
        # Kastelli prompt vaatii **TOIMITUSSISÄLTÖ START** ja **TOIMITUSSISÄLTÖ END** tunnisteet
        kastelli_syote = f"**TOIMITUSSISÄLTÖ START**\n{puhdistettu_toimitussisalto}\n**TOIMITUSSISÄLTÖ END**"
        
        ikkunatiedot_kokonaisuudessa = openai_api_kysely(PROMPT_KASTELLI_POIMI_IKKUNATIEDOT_TXT, kastelli_syote)
        ...
        if ikkunatiedot_kokonaisuudessa and ikkunatiedot_kokonaisuudessa.strip():
            ...
            print("❌ VIRHE: Kastelli ikkuna-analyysi palautti tyhjän tuloksen!")
            print("🔍 Tarkista API-avain ja verkko-yhteys")
            kirjoita_txt_tiedosto("VIRHE: Tyhjä tulos Kastelli ikkuna-analyysissä", f"terminaalitulosteet/kastelli_ikkunatiedot_raaka_{toimitussisalto_id}.txt")
        
        print(f"\n🔍 KASTELLI API-KYSELY: Ikkunat JSON-muotoon")
        ikkunat_json = openai_api_kysely(PROMPT_KASTELLI_RYHMITELLE_VALITUT_IKKUNATIEDOT_JSON_MUOTOON, ikkunatiedot_kokonaisuudessa)
        ...
        # Debug: Tarkista JSON-muoto
        print(f"🔍 Kastelli ikkunat JSON (ensimmäiset 200 merkkiä): {ikkunat_json_puhdistettu[:200]}...")
        
        # Tarkista että JSON ei ole tyhjä
        if not ikkunat_json_puhdistettu or ikkunat_json_puhdistettu.strip() == "":
            print("❌ VIRHE: Kastelli ikkunat JSON on tyhjä!")
            return
        
        lisaa_ikkunat_kantaan(ikkunat_json_puhdistettu, toimitussisalto_id)
        ...
        ulko_ovet_teksti = openai_api_kysely(PROMPT_KASTELLI_POIMI_ULKO_OVI_TIEDOT_TXT, kastelli_syote)
        ...
        if not ulko_ovet_teksti or ulko_ovet_teksti.strip() == "":
            ...
            ulko_ovet = openai_api_kysely_ulko_ovet(PROMPT_KASTELLI_ULKO_OVI_TIEDOT_LUOKKAMUOTOON, ulko_ovet_teksti)
       ...
       #---------------------------------------     Kastelli vali-ovet kantaan      ----------------------------------------
        ...
        valio_ovet = openai_api_kysely(PROMPT_KASTELLI_POIMI_VALIOVITIEDOT_TXT, kastelli_syote)
        ...
        if not valio_ovet or valio_ovet.strip() == "":
            ...
            valio_ovet = openai_api_kysely(PROMPT_KASTELLI_ANNA_VALIOVIMALLIT_TXT, valio_ovet)
        ...
        toimitussisalto_tuotteet = openai_api_kysely_nelja_parametria(PROMPT_KASTELLI_POIMI_TUOTTEET_TXT, kastelli_syote, tuotteet)
...
def run_designtalo():
        ...
        ikkunatiedot_designtalo = openai_api_kysely(PROMPT_DESIGNTALO_POIMI_IKKUNATIEDOT_TXT, puhdistettu_toimitussisalto)
        kirjoita_txt_tiedosto(ikkunatiedot_designtalo, IKKUNATIEDOT_DESIGNTALO_KOKONAISUUDESSA_TXT)
        ikkunat_json_designtalo = openai_api_kysely(PROMPT_DESIGNTALO_RYHMITELLE_VALITUT_IKKUNATIEDOT_JSON_MUOTOON, ikkunatiedot_designtalo)
        ...
        ulko_ovi_tiedot_designtalo = openai_api_kysely(PROMPT_DESIGNTALO_POIMI_ULKO_OVI_TIEDOT_TXT, puhdistettu_toimitussisalto)
        kirjoita_txt_tiedosto(ulko_ovi_tiedot_designtalo, ULKO_OVI_TIEDOT_DESIGNTALO_KOKONAISUUDESSA_TXT)
        ulko_ovi_json_designtalo = openai_api_kysely(PROMPT_DESIGNTALO_ULKO_OVI_TIEDOT_JSON_MUOTOON, ulko_ovi_tiedot_designtalo)
        ...
        valiovi_tiedot_designtalo = openai_api_kysely(PROMPT_DESIGNTALO_POIMI_VALIOVITIEDOT_TXT, puhdistettu_toimitussisalto)
        kirjoita_txt_tiedosto(valiovi_tiedot_designtalo, VALIOVI_TIEDOT_DESIGNTALO_KOKONAISUUDESSA_TXT)
        valiovi_json_designtalo = openai_api_kysely(PROMPT_DESIGNTALO_ANNA_VALIOVIMALLIT_TXT, valiovi_tiedot_designtalo)
